  ...
  <div id="sidebar-nav" class="sidebar">
    <div class="sidebar-scroll">
      ...
      <ul class="nav">
          ...
          <li><a href="analyse.php?id_patient=<?php echo $id_patient;?>&id_app=<?php echo $id_appo;?>" class="active"><i class="fa fa-flask" aria-hidden="true"></i><span>Analyse</span></a></li>
...
?>
  <div class="row mb-4">
    <div class="col-sm-3 themed-grid-col"><strong>ID =       </strong> <?php echo $id_appo ."/". $id_patient; ?> </div>
...
<div style="background-color: #fff;" class="jumbotron">
    ...
    <div class="form-row">
              <div class="form-group col-md-4">
                <label style="font-family:  'Open Sans', sans-serif; font-weight: 400; font-size: 20px;">Name is Analysis</label>
                <input type="text" class="form-control input_style" name="nameanalysis" placeholder="Enter Name Medicine" >
              </div>
          </div>
        <button style="margin-top: 32px;" type="submit" name="addanalysis" class="btn btn-primary btn-lg ">Add Analysis </button>
...
<div class="panel">
	<div class="panel-heading">
		...
		<h3 style="color: #000;"  class="panel-title">Request medical Analyse of a patient</h3>
	</div>
			<div class="panel-body">
        <form action="Code PHP/include/examinig/index_analyse.php" method="POSt">
...
<?php
$sql2 = "SELECT * FROM `analyse`  GROUP BY `name_ana`";
  $result_run = mysqli_query($conn, $sql2);
    if (mysqli_num_rows($result_run) > 0) {
      // output data of each row
      while($row = mysqli_fetch_assoc($result_run)) {   
?>  
        <label><input type="checkbox" name="analyses[]" value="<?php echo $row["name_ana"];?>" class="form-check-input"><?php echo $row["name_ana"];?></label><br>
<?php
    }
  } else {
    echo "0 results";
...
<!-- END Code PHP Select info analyse -->
        ...
        <button type="submit" name="analyse" class="btn btn-primary btn-lg btn-block">Analyse</button>
      </form>
