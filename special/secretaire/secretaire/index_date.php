...
// Upsate Appointement **************************
    if (isset($_POST["rdvpatient"])) {
     ...
     $sql= "UPDATE `rendez_vous` SET `ID_sec`='$ID_sec', `objt_rdv`= '$objt_rdv' ,`date_rdv`= '$date_rdv', `date_rdv2` ='$date_toda'  WHERE  `ID_RDv` = '$id_rdv' ";
         if (mysqli_query($conn, $sql)) {
           echo '  <script>
           window.alert("The appointement has benn modified for someone ");
       </script>';
         } else {
